$(document).ready(function(){
    ...
    $(".find_pw_ok_btn").click(function () {
        ...
        if($('input[name=type_chk]:checked').val()==='이메일'){
            ...
            if (result_chk_insert) {
                chkMemberIsEmail(inserted_email, inserted_code);
            } else {
                //Todo : 비정상 실행 결과
            }
        } else{
            ...
            if (result_chk_insert) {
                chkMemberIsTel(inserted_tel1, inserted_tel2, inserted_tel3, inserted_code);
            } else {
                //Todo : 비정상 실행 결과
            }
        }
    ...
    /* 1. 검증 */
    function chkMemberIsEmail(inserted_email, inserted_code) {
        if (checkDBValuesEmail(inserted_email, inserted_code) === 1) { // 성공창
            // To do : 파라미터 값을 넘겨주기
            window.open("/findPwResult.mdo?a=1", "mg_find_pw_result", "width=600px, height=300px")
        } else {
            window.open("/findPwResult.mdo?a=2", "mg_find_pw_result", "width=600px, height=300px")
        }
    }

    /* 2. DB 값 조회 및 비교 */
    function checkDBValuesEmail(inserted_email, inserted_code) {
    ...
    function chkInsertTel() {
        ...
        if ($("#find_phone_value_1").val().trim() !== ''
    ...
    /* 1. 검증 */
    function chkMemberIsTel(inserted_tel1, inserted_tel2, inserted_tel3, inserted_code) {

        if (checkDBValuesTel(inserted_tel1, inserted_tel2, inserted_tel3, inserted_code) === 1) { // 성공창
            // To do : 파라미터 값을 넘겨주기
            window.open("/findPwResult.mdo?a=1", "mg_find_pw_result", "width=600px, height=300px")
        } else {
            window.open("/findPwResult.mdo?a=2", "mg_find_pw_result", "width=600px, height=300px")
    ...
    /* 2. DB 값 조회 및 비교 */
    function checkDBValuesTel(inserted_tel1, inserted_tel2, inserted_tel3, inserted_code) {
        // To do : DB에 들어가서 값을 조회하는 코드 작성해주기
        return 1;
    }
...
/*$('.login_btn').click(function() {
    window.opener.location.href="/login.mdo";
    window.close();
})*/
